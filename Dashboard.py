...
if st.session_state['login']:
    ...
    if st.session_state['page'] == 'home':
        ...
        st.markdown("<h6 style='text-align: right; color: grey;'>CapEx Dashboard</h6>", unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            <div style="display: flex; justify-content: space-between;">
                ...
                <div style="background-color: #d4edda; padding: 10px; margin: 5px; border-radius: 5px;">
                    <h4 style="color: green;">Total Cost Impact</h4>
                    <p>${total_cost_impact:.2f} Million</p>
                </div>
            </div>
            """, 
        ...
        # Plot 1: Top 5 Markets by Count
        top_5_markets_count = filtered_df['Market'].value_counts().nlargest(5).reset_index()
        top_5_markets_count.columns = ['Market', 'Count']
        fig1 = px.bar(top_5_markets_count, x='Market', y='Count', title='Top 5 Markets by Count')

        # Plot 2: Historical Spend vs. Updated Projected Spend (Top 5 Markets)
        top_5_markets_spend = filtered_df[filtered_df['Market'].isin(top_5_markets_count['Market'])]
        ...
        # Plot 3: Distribution of Predicted ROI
        fig3 = px.histogram(filtered_df, x='Predicted_ROI', title='Distribution of Predicted ROI')

        # Plot 4: NPV vs. ROI
        fig4 = px.scatter(filtered_df, x='NPV_Million', y='Predicted_ROI', 
        ...
        # Show DataFrames
        st.write("Market-wise DataFrame", grouped_df)
        st.write("Overall DataFrame", df)
